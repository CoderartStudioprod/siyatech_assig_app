...
class HackerNewsRepository {
  ...
  Future<List<int>> fetchTopStoryIds() async {
    ...
    if (response.statusCode != 200) throw Exception('Failed to load top story IDs');
    return List<int>.from(response.data);
  }

  Future<Post> fetchPost(int id) async {
    final response = await dio.get('https://hacker-news.firebaseio.com/v0/item/$id.json?print=pretty');
